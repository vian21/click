...
// loadSounds loads all the metronome sounds from the DOM
function loadSounds() {
  let cla_p = audioCtx.createMediaElementSource(
    document.getElementById("cla_p")
  );
  let cla_s = audioCtx.createMediaElementSource(
    document.getElementById("cla_s")
  );
  let cli_ps = audioCtx.createMediaElementSource(
    document.getElementById("cli_ps")
  );
  let woo_p = audioCtx.createMediaElementSource(
    document.getElementById("woo_p")
  );
  let woo_s = audioCtx.createMediaElementSource(
    document.getElementById("woo_s")
  );

  //connect audio to gain node
  cla_p.connect(gainNode);
  cla_s.connect(gainNode);
  cli_ps.connect(gainNode);
  woo_p.connect(gainNode);
  woo_s.connect(gainNode);

  //connect gainNode to main audio context/speakers
...
function addListeners() {
  ...
  document
    .getElementById("volumeController")
